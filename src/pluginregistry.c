...
static struct platch_obj_cb_data *get_cb_data_by_channel(struct plugin_registry *registry, const char *channel)
{
    struct platch_obj_cb_data *data;
...
struct plugin_registry *plugin_registry_new(struct flutterpi *flutterpi)
{
    ...
    ok = cpset_init(&reg->callbacks, 128);
    if (ok != 0) {
...
int plugin_registry_on_platform_message(FlutterPlatformMessage *message)
{
    ...
    ok = data_copy.callback(
      (char *)message->channel, &object, (FlutterPlatformMessageResponseHandle *)message->response_handle); //, data->userdata);
    if (ok != 0) {
