...
/* #define LOG_ERROR(s, ...)                                       \ */
/*     do {                                                        \ */
/*         printf("[ERROR] (AudioPlayer) " s "\n", ##__VA_ARGS__); \ */
/*     } while (0) */
#define LOG_INFO(s, ...)                                       \
    do {                                                       \
        printf("[INFO] (AudioPlayer) " s "\n", ##__VA_ARGS__); \
    } while (0)
...
void AudioPlayer_OnPositionUpdate(AudioPlayer* self)
{
    LOG_INFO("ON_POSITION_UPDATE");
    if (self->_channel) {
        /* g_autoptr(FlValue) map = fl_value_new_map(); */
        /* fl_value_set_string(map, "playerId", fl_value_new_string(_playerId.c_str())); */
        /* fl_value_set_string(map, "value", fl_value_new_int(GetPosition())); */
        /* fl_method_channel_invoke_method(self->_channel, "audio.onCurrentPosition", map, nullptr, nullptr, nullptr); */
        platch_call_std(
          self->_channel,
...
{
    LOG_INFO("SetLooping(%s)", isLooping ? "true" : "false");
...
void AudioPlayer_clear(AudioPlayer* self)
{
    free(self->playerId);
    free(self->_channel);
}
...
{
    if (!self->_isInitialized) {
        LOG_INFO("Resume: Not Initialized");
        return;
    }
    LOG_INFO("[%s] Resume: SET STATE PLAYING", self->playerId);
...
{
    LOG_INFO("[%s]SetVolume(%g)", self->playerId, volume);
    if (volume > 1) {
        volume = 1;
    } else if (volume < 0) {
        volume = 0;
